...
int OnInit()
{
   ...
   // Ribbon plots (0-6): EMPTY_VALUE定義 — 未計算領域を描画しない
   for(int p = 0; p <= 6; p++)
      PlotIndexSetDouble(p, PLOT_EMPTY_VALUE, EMPTY_VALUE);
   // Candle plot (9)
   PlotIndexSetDouble(9, PLOT_EMPTY_VALUE, EMPTY_VALUE);

   // 全描画バッファをEMPTY_VALUEで初期化 — TF切替時のゴミ値・0.0残留を防止
   ArrayInitialize(g_ema1,  EMPTY_VALUE);
   ArrayInitialize(g_ema2a, EMPTY_VALUE);
   ArrayInitialize(g_ema2b, EMPTY_VALUE);
   ArrayInitialize(g_ema3a, EMPTY_VALUE);
   ArrayInitialize(g_ema3b, EMPTY_VALUE);
   ArrayInitialize(g_ema4a, EMPTY_VALUE);
   ArrayInitialize(g_ema4b, EMPTY_VALUE);
   ArrayInitialize(g_ema5a, EMPTY_VALUE);
   ArrayInitialize(g_ema5b, EMPTY_VALUE);
   ArrayInitialize(g_ema6a, EMPTY_VALUE);
   ArrayInitialize(g_ema6b, EMPTY_VALUE);
   ArrayInitialize(g_ema7a, EMPTY_VALUE);
   ArrayInitialize(g_ema7b, EMPTY_VALUE);
   ArrayInitialize(g_ema8,  EMPTY_VALUE);
