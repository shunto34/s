                ...
                const int &spread[])
{
   ...
   if(fullRecalc || newBar)
   {
      ...
      if(fullRecalc)
      {
         ...
         // Prevent stale historical notifications on first newBar after load
         if(rates_total > 0)
         {
            g_lastNotifyTime  = time[rates_total - 1];
            g_lastExitNotify  = time[rates_total - 1];
            g_lastMSSNotify0  = time[rates_total - 1];
            g_lastMSSNotify1  = time[rates_total - 1];
            g_lastMSSNotify2  = time[rates_total - 1];
         }
      }
      ...
      // Post-loop notification: track the LATEST signal for deferred sending
      int pendSigI = -1;
      bool pendSigBull = false;
      int pendSigScore = 0;
      int pendExI = -1;
      bool pendExBullPos = false;
      int pendExScore = 0;
      string pendExTxt = "";
      ...
      for(int i = startSig; i < rates_total; i++)
      {
         ...
         if(passRange && ((nT == 1 && prevMaster != 1) || (nT == -1 && prevMaster != -1)))
         {
            ...
            // Track latest signal for post-loop notification
            if(prev_calculated > 0 && time[i] > g_lastNotifyTime)
            {
               pendSigI = i;
               pendSigBull = isBull;
               pendSigScore = score;
            ...
            && (i - lastExitBar) >= InpCooldownBars)
         {
            ...
            if(exitScore >= InpExitThreshold)
            {
               ...
               // Track latest EXIT for post-loop notification
               if(prev_calculated > 0 && time[i] > g_lastExitNotify)
               {
                  pendExI = i;
                  pendExBullPos = isBullPos;
                  pendExScore = exitScore;
                  pendExTxt = eTxt;
               }
            }
         }
      ...
      // === POST-LOOP NOTIFICATIONS ===
      // Send notifications for the LATEST signal detected in the loop.
      // This guarantees delivery regardless of which bar index the signal is on.
      if(pendSigI >= 0)
      {
         ...
         if(conf > 99) conf = 99;
         bool notifyThis = pendSigBull ? InpNotifyBull : InpNotifyBear;
         SendSignalAlert(pendSigBull ? "BULL " + r2 : "BEAR " + r2,
                         close[pendSigI], notifyThis, conf);
      }
      if(pendExI >= 0)
      {
         g_lastExitNotify = time[pendExI];
         string tfStr = EnumToString(Period());
         StringReplace(tfStr, "PERIOD_", "");
         string exitMsg = StringFormat("[%s] \x25C6 %s %s @ %s [%d%%] | %s | %s",
            _Symbol, pendExTxt, pendExBullPos ? "LONG TP" : "SHORT TP",
            DoubleToString(close[pendExI], _Digits), pendExScore, tfStr,
            TimeToString(TimeCurrent(), TIME_DATE|TIME_MINUTES));
