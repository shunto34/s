...
//|  ATR Supertrend Filter, RSI+Stoch Momentum, Continuation Signals |
//+------------------------------------------------------------------+
...
input int    InpFlipPeriod    = 10;       // Supertrend ATR Period
input bool   InpShowFlipLine  = true;     // Show Supertrend Line on Chart
input bool   InpUseMomentum   = true;     // RSI+Stochastic Momentum Filter
...
input bool   InpContSignals   = true;     // Continuation Signals (pullback re-entry)
input int    InpContCooldown  = 5;        // Continuation Min Bars After Entry
...
//--- GEXR Fusion: Supertrend + Momentum
...
//+------------------------------------------------------------------+
void CalcSupertrend(int rates_total, const double &high[], const double &low[],
                    const double &close[], const double &flipATR[], int flipCopied)
{
   ArrayResize(g_flipLine, rates_total);
   ArrayResize(g_flipDir,  rates_total);
   if(flipCopied < 2) return;

   g_flipDir[0]  = 1;
   g_flipLine[0] = close[0];

   for(int i = 1; i < rates_total; i++)
   {
      double aVal = (i < flipCopied && flipATR[i] > 0) ? flipATR[i] : flipATR[MathMin(i, flipCopied - 1)];
      double hl2  = (high[i] + low[i]) / 2.0;
      double up   = hl2 + InpFlipMult * aVal;
      double dn   = hl2 - InpFlipMult * aVal;

      if(g_flipDir[i - 1] == 1 && dn < g_flipLine[i - 1])
         dn = g_flipLine[i - 1];
      if(g_flipDir[i - 1] == -1 && up > g_flipLine[i - 1])
         up = g_flipLine[i - 1];

      if(g_flipDir[i - 1] == 1)
      {
         if(close[i] < g_flipLine[i - 1])
         { g_flipDir[i] = -1; g_flipLine[i] = up; }
         else
         { g_flipDir[i] = 1;  g_flipLine[i] = dn; }
      }
      else
      {
         if(close[i] > g_flipLine[i - 1])
...
bool IsActiveSession(datetime t)
{
   ...
   MqlDateTime dt;
   TimeToStruct(t, dt);
   if(InpActiveStart < InpActiveEnd)
      return (dt.hour >= InpActiveStart && dt.hour < InpActiveEnd);
   return (dt.hour >= InpActiveStart || dt.hour < InpActiveEnd);
}
...
int OnInit()
{
   ...
   // GEXR Fusion handles
   if(InpUseFlipFilter || InpShowFlipLine)
   {
      g_hFlipATR = iATR(_Symbol, PERIOD_CURRENT, InpFlipPeriod);
      if(g_hFlipATR == INVALID_HANDLE) return(INIT_FAILED);
   }
   if(InpUseMomentum)
   {
      ...
      if(g_hStoch == INVALID_HANDLE) return(INIT_FAILED);
   }
   ...
   IndicatorSetString(INDICATOR_SHORTNAME, "FAD APEX v5");
...
void OnDeinit(const int reason)
{
   ...
   if(g_hFlipATR != INVALID_HANDLE) IndicatorRelease(g_hFlipATR);
                ...
                const int &spread[])
{
   ...
   if(fullRecalc || newBar)
   {
      ...
      // GEXR Fusion: copy RSI, Stochastic, Supertrend ATR
      double rsiBuf[], stochKBuf[], stochDBuf[], flipATRBuf[];
      ArraySetAsSeries(rsiBuf, false);
      ArraySetAsSeries(stochKBuf, false);
      ArraySetAsSeries(stochDBuf, false);
      ArraySetAsSeries(flipATRBuf, false);
      int rsiCopied = 0, stochCopied = 0, flipCopied = 0;
      if(InpUseMomentum && g_hRSI != INVALID_HANDLE)
      ...
      {
         flipCopied = CopyBuffer(g_hFlipATR, 0, 0, rates_total, flipATRBuf);
         CalcSupertrend(rates_total, high, low, close, flipATRBuf, flipCopied);
         DrawSupertrendLine(startSig, rates_total, time);
      }
      ...
      for(int i = startSig; i < rates_total; i++)
      {
         ...
         if(passRange && ((nT == 1 && prevMaster != 1) || (nT == -1 && prevMaster != -1)))
         {
            ...
            // --- Factor 5: ATR Supertrend direction agreement ---
            bool flipAligned = true;
            if(InpUseFlipFilter && flipCopied > 0 && i < ArraySize(g_flipDir))
            {
               flipAligned = (isBull && g_flipDir[i] == 1) || (!isBull && g_flipDir[i] == -1);
               if(flipAligned) score++;
            }

            // --- Factor 6: RSI momentum alignment ---
            bool rsiAligned = true;
            if(InpUseMomentum && rsiCopied > i)
            {
               rsiAligned = (isBull && rsiBuf[i] > 50) || (!isBull && rsiBuf[i] < 50);
               if(rsiAligned) score++;
            }

            // --- Factor 7: Stochastic momentum alignment ---
            bool stochAligned = true;
            if(InpUseMomentum && stochCopied > i)
            {
               bool stBull = (stochKBuf[i] > stochDBuf[i]) && (stochKBuf[i] < 80);
               bool stBear = (stochKBuf[i] < stochDBuf[i]) && (stochKBuf[i] > 20);
               stochAligned = (isBull && stBull) || (!isBull && stBear);
               if(stochAligned) score++;
            }

            // Session filter penalty
            bool inSession = IsActiveSession(time[i]);
            if(!inSession && InpSessionFilter) score--;

            // --- Sensitivity gating ---
            bool passFilter = true;
            if(InpSensitivity == SENS_CONSERVATIVE)
            {
               if(!flipAligned || !rsiAligned || !stochAligned) passFilter = false;
               if(score < 4) passFilter = false;
            }
            else if(InpSensitivity == SENS_BALANCED)
            {
               if(!flipAligned && !rsiAligned) passFilter = false;
               if(score < 2) passFilter = false;
            }
            else
            {
               if(score < 1) passFilter = false;
            }

            if(passFilter)
            {
            // Rank: S(5+), A(3-4), B(0-2)  (out of 7 factors)
            ...
            if(isBull)
            {
               ...
               int dotSz = (score >= 5) ? 32 : (score >= 3) ? 28 : 24;
               int txtSz = (score >= 5) ? 12 : (score >= 3) ? 10 : 9;
               CreateSignalDot("DotBull" + IntegerToString(i),
                  time[i], low[i], dotC, false, dotSz);
               string bTxt = (score >= 5) ? "BULL S \x2605" : (score >= 3) ? "BULL A" : "BULL B";
            ...
            else
            {
               ...
               string bTxt = (score >= 5) ? "BEAR S \x2605" : (score >= 3) ? "BEAR A" : "BEAR B";
               double lblOfs = (pixToPrice > 0) ? (dotSz + 5) * pixToPrice
                             : ((atrCopied > i && atrBuf[i] > 0) ? atrBuf[i] * 0.7 : _Point * 15);
            ...
            // Confidence: 7-factor system + ADX bonus
            if(i >= rates_total - 2 && prev_calculated > 0 && time[i] > g_lastNotifyTime)
            {
               g_lastNotifyTime = time[i];
               int conf = 15 + score * 11;
               if(flipAligned) conf += 5;
               if(rsiAligned)  conf += 4;
               if(stochAligned) conf += 3;
               if(inSession)   conf += 3;
               if(adxCopied > i && adxBuf[i] > 30) conf += 4;
               if(adxCopied > i && adxBuf[i] > 40) conf += 3;
               if(conf > 99) conf = 99;
               bool notifyThis = isBull ? InpNotifyBull : InpNotifyBear;
               SendSignalAlert(isBull ? "BULL " + rank : "BEAR " + rank,
            ...
            && (i - g_lastContBar) >= InpCooldownBars && passRange)
         {
            ...
            bool ribbonOK = cBull ? (g_ed0[i] > g_ed7[i]) : (g_ed0[i] < g_ed7[i]);
            bool touched = false;
            if(ribbonOK && i >= 2)
            {
               double mid = (g_ed2[i - 1] + g_ed5[i - 1]) / 2.0;
               if(cBull)
                  touched = (low[i - 1] <= mid && close[i] > g_ed0[i] && close[i - 1] <= g_ed2[i - 1]);
               else
                  touched = (high[i - 1] >= mid && close[i] < g_ed0[i] && close[i - 1] >= g_ed2[i - 1]);
            }
            if(touched)
            {
               bool flipOK = true;
               if(InpUseFlipFilter && flipCopied > 0 && i < ArraySize(g_flipDir))
                  flipOK = (cBull && g_flipDir[i] == 1) || (!cBull && g_flipDir[i] == -1);
               bool momOK = true;
               if(InpUseMomentum && rsiCopied > i)
                  momOK = (cBull && rsiBuf[i] > 45) || (!cBull && rsiBuf[i] < 55);

               if(flipOK && momOK)
               {
                  g_lastContBar = i;
                  color cntC = cBull ? C'100,230,180' : C'230,120,120';
                  int cSz = 22;
                  int cTxtSz = 9;
                  string cTxt = cBull ? "CONT \x25B2" : "CONT \x25BC";
                  double pr = cBull ? low[i] : high[i];
                  CreateSignalDot("DotCont" + IntegerToString(i), time[i], pr, cntC, !cBull, cSz);
                  double cOfs = (pixToPrice > 0) ? (cSz + 5) * pixToPrice
                              : ((atrCopied > i && atrBuf[i] > 0) ? atrBuf[i] * 0.7 : _Point * 15);
                  double cPr = cBull ? (pr - cOfs) : (pr + cOfs);
                  CreateSignalLabel("SigCont" + IntegerToString(i), time[i], cPr, cTxt, cntC, !cBull, cTxtSz);

                  if(i >= rates_total - 2 && prev_calculated > 0 && time[i] > g_lastNotifyTime)
                  {
                     g_lastNotifyTime = time[i];
                     int cConf = 50;
                     if(flipOK && InpUseFlipFilter) cConf += 15;
                     if(momOK && InpUseMomentum) cConf += 10;
                     if(cConf > 99) cConf = 99;
                     bool notifyCont = cBull ? InpNotifyBull : InpNotifyBear;
                     SendSignalAlert(cBull ? "CONT BULL" : "CONT BEAR", close[i], notifyCont, cConf);
                  }
               }
            }
         }
