...
Python訓練スクリプトは**単一時間足のみ**でシグナル条件を再現します。
MQL5 側で使われている **M5/M15/H1 MTF構造 (BOS/MSS) の厳密な再現は省略**し、
EMA配列による代替指標で MTF 一致度を近似しています。

### 影響
- モデルの予測精度は MQL5 の実際のシグナル条件と**完全には一致しない**
- 実運用では AUC 0.55〜0.65 程度が現実的な上限
- 高精度を求める場合は、MT5 Python API で実際に M5/M15/H1 データを取得して訓練
  （将来の拡張ポイント）

### 推奨される運用
1. **Phase A（自己学習）を主軸に使う** — 実データで自動チューニング
2. **Phase B（AI%）は補助指標として参照** — 閾値フィルタには使わない方が安全
3. `InpAIMinWinRate = 0`（フィルタ無効）から試す
